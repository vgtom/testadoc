...
export const UserMenuItems = ({
  ...
  type MenuItem = {
    name: string;
    to: (typeof routes)[keyof typeof routes]["to"];
    icon: ReactNode;
  };
  ...
  const landingAndAdminPanelOnlyMenuItems: MenuItem[] = useMemo(
    () => [
  ...
  return (
    <>
      <ul
        className={cn(
          "flex flex-col gap-5 border-b border-stroke py-4 dark:border-strokedark",
          {
      ...
      >
        {path === landingPagePath || path === adminDashboardPath ? (
          <>
            {landingAndAdminPanelOnlyMenuItems.map((i) => (
              <li key={i.to}>
                <WaspRouterLink
                  to={i.to as any}
